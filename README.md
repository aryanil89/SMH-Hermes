...
| Phone's role | Approval terminal (`phone.html`) + Telegram client + **challenge notification pushed to Telegram** (text only, deliberately no photo; fire-and-forget, silent no-op when unconfigured) — **plus a measured on-phone NPU benchmark** (2026-08-06, `genie-t2t-run` one-shot over `adb`; the phone stayed a working approval terminal throughout and all bench artifacts were deleted after) | On-phone **serving** on the 8 Elite and the "failover brain" demo beat — the benchmark proved the model runs there; serving it is the next rung |
