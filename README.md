...
|---|---|---|
| LLM inference | Qwen3-4B-Instruct-2507 Q4_0 on the **laptop's** X Elite NPU via GenieX — measured, serving the agent. **Same model measured on the phone's 8 Elite NPU** (2026-08-06): pre-compiled AI Hub bundle (w4a16, ctx 4096) via `genie-t2t-run` over `adb` — **prefill 1,918 ± 16.9 tok/s, decode 23.1 ± 1.3** — two Hexagon NPUs, one table, every config delta labeled ([RESULTS.md § Phone benchmark](llm-serving-bench/RESULTS.md#phone-benchmark-snapdragon-8-elite--2026-08-06)). **And the failover brain is wired** (verified live 2026-08-06): a dead GenieX — TCP connect refused, never an HTTP probe — routes the Telegram question to the phone over `adb`, answer delivered to Telegram + wall labeled degraded, **12.0 s** message→delivered (n=1). The **phone leg** of that is repeated: **7.1 ± 0.7 s over n=5** (2026-08-07), of which **3.83 ± 0.04 s is model load paid on every question** — so the leg is ~4.0 s fixed + ~0.04 s per generated token ([hermes-hooks/README.md](hermes-hooks/README.md)) | On-phone **serving** (a persistent endpoint — the failover path is a one-shot CLI run per question, which is exactly where that fixed 4 s comes from) |
...
| On-board activity inference | **SmolLM2-135M runs on the UNO Q itself** (CPU — the board's Adreno 702 GPU was found, measured and rejected: ~32× slower decode), correlating recent sensor history into `activity-*` log lines. A fresh line reaches the wall's device feed as an **"AI:" row** and is folded into the watchdog's Telegram page — built + verified 2026-08-06 ([docs/ONDEVICE_ACTIVITY.md](docs/ONDEVICE_ACTIVITY.md)). **And the 4B tier now sees them too** (2026-08-07): `get_incident_assessment` reports the newest inference and says it in the summary the model relays — *"Also observed (not scored): the board's own model inferred 'Person left room' 60 minutes ago"*. **Never scored**: the sentence is built after the risk and confidence arithmetic has run and never touches the evidence array, so a test asserts the verdict is bit-for-bit identical either side of an activity. Two models on two devices, one verdict, and the small one cannot move the big one's numbers | Level-based leak threshold, demoted (Button C is the trigger — `distance_mm` no longer rides the sensor tick, and reflashing working firmware before the demo is the worse risk) |
| Infrastructure telemetry | Network / storage / compute are **simulated**, and say so in every row: each node carries `source: "real" \| "mock"`. **One node is real** (2026-08-07) — `host-01` is this laptop, reporting actual CPU (a 200 ms delta, because `os.cpus()` counters are cumulative since boot and `os.loadavg()` is `[0,0,0]` on Windows), memory and uptime through the same seam as the mocks. It declines rather than fabricates: unreadable counters return `undefined` and the fleet is simply all-simulated, and `thermalThrottle` is **absent, not `false`**, because nothing here can measure it. Deliberately **not** scored by `get_incident_assessment`, so risk stays reproducible | Real fleet telemetry (a rack that exists). The simulators stay thermally coupled to the real sensor on purpose — see [Thermal coupling](mcp-tools/#thermal-coupling--why-the-correlation-is-real) — because two uncoupled families are what make the diagnosis falsifiable |
